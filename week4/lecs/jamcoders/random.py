...
# Insertion order is the canonical draw order -- see draw_cup_as_stacked_bars.
COLORS_MAP = {
    "red": "#e74c3c",
    "green": "#2ecc71",
    "yellow": "#f1c40f",
...
def sample_from_dict(prob_dict):
    ...
    Example:
        >>> sample_from_dict({"red": 0.4, "purple": 0.3, "green": 0.2, "yellow": 0.1})
    ...
    for v in prob_dict.values():
        # bool is a subclass of int, so reject it explicitly -- True is not a probability.
...
def visualize(bag, sample_sizes):
    ...
    Args:
        bag: List of color strings (e.g., ["red", "red", "purple", ...])
    ...
    def draw_cup_as_stacked_bars(ax, proportions):
        ...
        # Draw in COLORS_MAP order rather than set(bag) order: set iteration order
        # isn't stable across runs, so the bars would reshuffle on every restart.
        colors = [c for c in COLORS_MAP if proportions.get(c, 0) > 0]
        y_current = cup_base
        ...
        for i, color in enumerate(colors):
            if i == len(colors) - 1:  # Last color present gets the remainder
        ...
        for color in colors:
            ...
            # Name the colour as well as the share, so the chart is readable
            # without having to tell the colours apart
            if proportions[color] > 0.05:
                mid_y = y_current + section_height / 2
                ax.text(0, mid_y, f'{color} {percentages[color]}%',
